...
GitHub Actions runs the same hooks as the `pre-commit` job in
[`.github/workflows/test.yml`](.github/workflows/test.yml). That was implicit
in the Cloud Agent environment: `.cursor/install.sh` installs `black`,
`isort`, and `flake8` via `.[dev]`, but does **not** install the
...
| | | `end-of-file-fixer` (`types: [file, python]`) |
| | | `trailing-whitespace` (`types: [file, python]`) |
| | | `mixed-line-ending` |
