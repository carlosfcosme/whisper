...
class LoopbackHTTPServer(HTTPServer):
    """HTTPServer that cannot listen on 0.0.0.0."""

    allow_reuse_address = True

    def __init__(self, server_address, RequestHandlerClass):
        host, port = server_address
        super().__init__((assert_loopback_bind(host), port), RequestHandlerClass)
...
def create_loopback_server(host=LOOPBACK_HOST, port=0):
    """Bind a tiny HTTP server on 127.0.0.1 only."""
    return LoopbackHTTPServer((host, port), _QuietHandler)


def bound_host(server) -> str:
    return server.socket.getsockname()[0]
