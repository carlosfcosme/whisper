...
def refuse_weight_auto_download(url: str) -> None:
    ...
    raise WeightDownloadError(
        ...
        "path ({} or CI). Place the file in the download "
        "root or pass a local checkpoint path to load_model().".format(
...
def refuse_non_localhost_bind(host: str) -> None:
    ...
    raise BindError(
        "Refusing bind to {!r}. Helper listeners must bind {}.".format(
...
def default_bind_host() -> str:
    ...
    Wildcard / public hosts are refused.
    """
...
def bind_localhost(sock, port=0) -> Tuple[str, int]:
    ...
    Use this instead of binding a wildcard or public interface.
    """
