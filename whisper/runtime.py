"""Sovereign Cloud Agent / CI runtime.

Three rules, all implemented here rather than implied:

- **CPU default** — ``default_device()`` is ``cpu`` (not ``cuda``-if-available).
- **No Hub download** — cache-miss weight pulls, including Hugging Face Hub, are refused.
- **Bind 127.0.0.1** — helper listeners use ``default_bind_host()``, never ``0.0.0.0``.
"""
...
def refuse_non_localhost_bind(host: str) -> None:
    """Refuse wildcard and non-loopback bind addresses (``0.0.0.0``, ``::``, LAN, public)."""
...
def default_bind_host() -> str:
    """Host for any helper listener. Sovereign default is ``127.0.0.1``.
