...
_HF_HUB_HOSTS = {"huggingface.co", "hf.co", "huggingface.com"}
_HF_HUB_SUFFIXES = (".huggingface.co", ".hf.co", ".huggingface.com")
...
def weight_auto_download_allowed() -> bool:
    """False when ``WHISPER_NO_WEIGHT_DOWNLOAD`` refuses cache-miss pulls.

    Hugging Face Hub URLs are refused even when this returns True.
    ``WHISPER_ALLOW_WEIGHT_DOWNLOAD=1`` is an explicit escape hatch for
    non-Hub official checkpoints.
    """
    if _env_truthy(ALLOW_WEIGHT_DOWNLOAD_ENV):
        return True
    if _env_truthy(NO_WEIGHT_DOWNLOAD_ENV) or _env_truthy(CPU_ONLY_ENV):
        return False
    return True


def refuse_weight_auto_download(url: str) -> None:
    """Refuse Hugging Face Hub URLs always; refuse other pulls when disabled."""
    ...
    raise WeightDownloadError(
        ...
        f"({NO_WEIGHT_DOWNLOAD_ENV} or {CPU_ONLY_ENV}). "
        "Place the file in the download root or pass a local checkpoint path."
    )
