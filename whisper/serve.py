...
class BindError(ValueError):
    """Raised when the bind host is not 127.0.0.1."""


def normalize_bind_host(host: str) -> str:
    """Accept only IPv4 loopback. ``localhost`` is rewritten to 127.0.0.1."""
    raw = (host or "").strip()
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        help="bind address (default: 127.0.0.1; IPv4 loopback only)",
    )
    parser.add_argument(
        "--port",
        type=int,
