"""Localhost-only HTTP serve path. Binds 127.0.0.1. No weights. No Hub."""
...
def main(argv: Optional[list] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Whisper localhost-only serve path (CPU, no weights, no Hub)."
    )
    parser.add_argument("--host", default=BIND_HOST, help=f"must be {BIND_HOST}")
