"""Localhost-only helper server. Does not load or pull model weights."""
...
from .sovereign import BIND_HOST, BIND_PORT, DEFAULT_DEVICE
...
def serve_bind_host(host: Optional[str] = None) -> str:
    ...
    addresses such as ``0.0.0.0`` and ``::`` are refused, as are LAN,
    public, and IPv6 hosts. The server binds 127.0.0.1 only.
    """
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Binding 0.0.0.0 is refused. No Hub and no weight pulls."
        ),
    )
    parser.add_argument(
        "--host",
