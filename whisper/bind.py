...
hosts are refused before ``socket.bind()``. This module is stdlib-only:
no WAN, no DNS, no model weights, no kernel modules, no BPF.
"""
...
_BLOCKED = frozenset({_UNSPECIFIED_V4, "::", "*", "[::]", "0", "::0"})
_LISTEN_STATE = "0A"
...
def _guarded_bind(self, address):
    ...
    if host is not None:
        require_loopback_host(host if host != "" else _UNSPECIFIED_V4)
...
def parse_proc_inet_table(text: str, *, ipv6: bool = False) -> List[ListenerRecord]:
    ...
    for line in lines[1:]:
        ...
        host = _ipv6_from_proc_hex(hex_ip) if ipv6 else _ipv4_from_proc_hex(hex_ip)
        rows.append(
            ListenerRecord(
                host=host,
