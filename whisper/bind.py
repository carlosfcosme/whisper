"""Loopback-only bind policy for serve/demo listeners.

Every listener binds ``127.0.0.1``. Empty host and all-interface
addresses are refused before ``bind()``. This module does not load
...
def require_bind_127_0_0_1(host: Optional[str] = None) -> str:
    ...
    if lowered in {_ALL_INTERFACES_V4, "::", "*", "[::]"}:
        raise BindError(
