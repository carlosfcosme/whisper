...
# Offline / no Hub / CPU: install must not fetch Whisper checkpoints.
...
# Formatter: Black (via .[dev]). Ruff is not present. See FORMAT.md.
...
python3 scripts/assert_sovereign.py --probe
