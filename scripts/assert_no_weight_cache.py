...
Used after `pip install` and after `pytest` so CI fails when the install/test
path downloaded checkpoints (Whisper Azure cache or Hugging Face Hub cache).
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if found:
        ...
        for path in found:
            sys.stderr.write("  {}\n".format(path))
        sys.stderr.write(
            "Keep HF_HUB_OFFLINE/WHISPER_OFFLINE set and do not call load_model "
            "with a Hub/Azure name unless the checkpoint is already on disk.\n"
        )
        return 1
