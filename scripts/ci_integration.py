...
* a demo/install/test server binds off 127.0.0.1
"""
...
def check_loopback_only(root: Path) -> List[str]:
    errors: List[str] = []
    demo = (root / "scripts" / "demo_server.py").read_text(encoding="utf-8")
    if (
        'DEFAULT_HOST = "127.0.0.1"' not in demo
        and "DEFAULT_HOST = '127.0.0.1'" not in demo
    ):
        errors.append("demo DEFAULT_HOST is not 127.0.0.1")
