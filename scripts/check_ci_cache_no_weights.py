...
def _is_forbidden_cache_path(path: str) -> bool:
    ...
    if not lowered or lowered.startswith("${{") and "pip-cache" in lowered:
        return False
...
def _cache_paths_from_text(text: str):
    paths = []
    ...
    for path in sorted(WORKFLOWS.glob("*.yml")) + sorted(WORKFLOWS.glob("*.yaml")):
        ...
        for cache_path in _cache_paths_from_text(text):
            if _is_forbidden_cache_path(cache_path):
                offenders.append(f"{path.relative_to(REPO)}: {cache_path}")
    if offenders:
        print("GitHub Actions cache must not include weights:", file=sys.stderr)
...
def check_runtime() -> int:
    ...
    if found:
        print("weight files present in CI cache directories:", file=sys.stderr)
        for line in found:
            print(f"  {line}", file=sys.stderr)
        return 1
    return 0
