...
Scans Cloud Agent install, GitHub Actions workflows, tests, and the demo
server. Library code may still download weights at user runtime; install and
test must not.
"""
...
# Only in install / workflow files: loading a named Hub/Azure checkpoint.
...
# Files that mention blocked URLs only to prove downloads are rejected.
...
# Tests that assert 0.0.0.0 is rejected.
...
ZERO_ADDR = "0.0.0.0"
# Quoted or assigned 0.0.0.0 — a docstring mentioning the refused address is OK.
BIND_RX = re.compile(
...
def find_download_violations(root: Path) -> List[str]:
    ...
    for path in iter_scan_paths(root):
        ...
        if _is_install_or_workflow(relpath):
            ...
            # Still flag real download APIs; skip host-only mentions.
            patterns = [
                (rx, label)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if binds:
        failed = True
        sys.stderr.write("ERROR: install/test/demo must bind to 127.0.0.1 only:\n")
        for item in binds:
            sys.stderr.write("  {}\n".format(item))
    if failed:
        return 1
