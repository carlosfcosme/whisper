#!/usr/bin/env python3
"""Fail CI if model weights or caches are committed.

Weights are loaded from a local cache (or downloaded outside CI). They
must not be tracked in git. Cache directories must not be tracked either.
This script is invoked from GitHub Actions and pre-commit.
"""
...
# Path components that indicate a cache tree (Hugging Face, torch, pytest, …).
...
def is_cache_path(relpath: str) -> bool:
...
def classify(relpath: str, size: int) -> Optional[str]:
    ...
    if is_cache_path(posix):
        return "committed cache path"
...
def main() -> int:
    ...
    if violations:
        sys.stderr.write("ERROR: model weights or caches must not be committed:\n")
        for relpath, reason in violations:
            sys.stderr.write("  {}: {}\n".format(relpath, reason))
        sys.stderr.write(
            "Download weights at runtime (or from a local cache); "
            "do not add them or cache trees to git.\n"
        )
        return 1
    sys.stdout.write("OK: no model weights or caches in the git tree\n")
    return 0
