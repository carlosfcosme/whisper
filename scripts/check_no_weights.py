#!/usr/bin/env python3
"""Fail CI if model weights, caches, or secret files are committed."""
...
SECRET_SUFFIXES = frozenset({".pem", ".p12", ".pfx"})
...
def classify(relpath: str, size: int) -> Optional[str]:
    ...
    if name in SECRET_NAMES or suffix in SECRET_SUFFIXES:
        return "committed secret or key ({})".format(name)
    if any(
        posix == prefix[:-1] or posix.startswith(prefix) for prefix in CACHE_PREFIXES
    ):
        return "committed cache or weight directory"
...
def main() -> int:
    ...
    if violations:
        ...
        sys.stderr.write("Load weights from a local cache; do not add them to git.\n")
        return 1
    sys.stdout.write("OK: no model weights, caches, or secrets in the git tree\n")
    return 0
