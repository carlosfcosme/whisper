...
Weights belong on the Hub or in a local cache, not in git. This script is the
source of truth for that rule and is invoked from GitHub Actions and pre-commit.
"""
...
# Anything larger than this is treated as a large binary, regardless of suffix.
# Official Whisper checkpoints start around 75 MiB; existing fixtures stay under
# this limit (largest tracked file is the multilingual notebook, ~5.7 MiB).
...
def main() -> int:
    ...
    if violations:
        ...
        sys.stderr.write(
            "Download weights at runtime (or from the Hub); do not add them to git.\n"
        )
        return 1
