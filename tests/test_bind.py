...
def test_all_interfaces_token_is_v4_unspecified():
    ...
    assert bind.UNSPECIFIED_V4 == ALL_INTERFACES
...
def test_bind_module_is_stdlib_only():
    source = (ROOT / "whisper" / "bind.py").read_text(encoding="utf-8")
...
def test_bind_tcp_listens_on_127_0_0_1():
    ...
    try:
        ...
        probe = bind.socket.create_connection((bind.LOOPBACK_HOST, port), timeout=1)
        probe.close()
    finally:
        sock.close()
