...
def test_cpu_is_the_default(monkeypatch):
    assert os.environ.get("CUDA_VISIBLE_DEVICES") == ""
    import torch

    from whisper.env_policy import DEVICE_ENV, resolve_device
...
def test_dummy_checkpoints_are_ignored_by_git():
    ...
    try:
        for rel in IGNORE_EXAMPLES:
            ...
            assert ignored.returncode == 0, rel
            if path.exists():
                continue
            parent = path.parent
            ...
            created.append(path)
        if created:
            rels = [str(path.relative_to(REPO_ROOT)) for path in created]
        ...
        for path in created:
            if path.exists():
                path.unlink()
        for parent in created_dirs:
            if parent.exists() and parent != REPO_ROOT and not any(parent.iterdir()):
                parent.rmdir()
