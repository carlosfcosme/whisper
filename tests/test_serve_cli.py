"""CLI / start-script integration: bind 127.0.0.1, refuse 0.0.0.0, no Hub."""
...
def _env(tmp_path, extra=None):
    env = os.environ.copy()
    env["WHISPER_ALLOW_WEIGHT_FETCH"] = "0"
...
def _no_weights(tmp_path):
    ...
    assert written == [], f"weight file written during serve: {written}"


def test_module_binds_127_and_serves_health(tmp_path):
...
def test_module_fails_on_wildcard_host(tmp_path):
    proc = subprocess.run(
