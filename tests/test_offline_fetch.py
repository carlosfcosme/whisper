...
from whisper.offline import OfflineFetchError, is_hub_host, refuse_remote_fetch
...
def test_named_model_does_not_hit_cdn(monkeypatch, tmp_path):
    ...
    with pytest.raises(OfflineFetchError, match="Refusing remote"):
...
def test_huggingface_hub_url_is_rejected(monkeypatch, tmp_path):
    ...
    with pytest.raises(OfflineFetchError, match="Hugging Face Hub"):
        whisper._download(hub_url, str(tmp_path), False)
    assert calls == []
    assert "huggingface_hub" not in sys.modules
    assert is_hub_host("huggingface.co")
    assert is_hub_host("cdn-lfs.huggingface.co")
...
def test_load_model_named_without_cache_skips_network(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(whisper.urllib.request, "urlopen", boom)
    with pytest.raises(OfflineFetchError):
        whisper.load_model("tiny", download_root=str(tmp_path / "whisper"))
    assert calls == []


def test_refuse_remote_fetch_mentions_loopback():
    with pytest.raises(OfflineFetchError, match="127.0.0.1"):
        refuse_remote_fetch(
...
def test_checksum_mismatch_does_not_redownload(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(whisper.urllib.request, "urlopen", boom)
    with pytest.raises(OfflineFetchError, match="checksum mismatch"):
        whisper._download(whisper._MODELS["tiny"], str(tmp_path), False)
    assert calls == []
