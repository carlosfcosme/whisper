...
def test_transcribe(model_name: str):
    cached = _cached_model_path(model_name)
    if cached is None:
        pytest.skip("local weights required; tests must not download models")

    # Load the on-disk file. Never pass an official model name (that downloads).
    model = whisper.load_model(cached)
