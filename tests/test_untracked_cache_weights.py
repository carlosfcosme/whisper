...
whisper.load_model() writes named checkpoints to ~/.cache/whisper or
$XDG_CACHE_HOME/whisper. --model_dir and download_root can point inside
this checkout. This module does not import whisper and does not download
weights.
"""
...
def test_gitignore_declares_cache_and_weight_paths():
    lines = {
        line.strip()
...
def test_cache_and_weight_paths_are_untracked():
    ...
    assert tracked == [], f"cache/weight/secret paths must stay untracked: {tracked}"


def test_git_index_has_no_cache_or_weight_files():
    ...
    offenders = [
        path
        for path in tracked
        if path.startswith(UNTRACKED_DIR_PREFIXES)
        or path.endswith(UNTRACKED_SUFFIXES)
        or path == ".env"
        or path.startswith(".env.")
    ]
    assert (
        offenders == []
    ), f"cache/weight/secret paths must stay untracked: {offenders}"


def test_gitignore_keeps_in_repo_cache_env_untracked():
    failed = [
        path
