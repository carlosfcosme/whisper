"""Ignore-coverage and git ls-files guard for cache/weight/coverage blobs."""
...
def test_ignore_coverage_artifacts():
    for path in (".coverage", "htmlcov/index.html", "coverage.xml"):
...
def test_no_hub_and_no_keys_in_guard():
    source = SCRIPT.read_text()
    lowered = source.lower()
    assert "huggingface_hub" not in lowered
    assert "hf_hub" not in lowered
    assert "api_key" not in lowered
    assert "field-brain" not in lowered
    assert "field_brain" not in lowered
