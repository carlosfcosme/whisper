"""CI grep + policy guard for loopback-only bind."""
...
def test_git_grep_script_rejects_zero_address():
    result = subprocess.run(
        ["bash", str(ROOT / "scripts" / "check_no_all_interfaces.sh")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert "0.0.0.0" not in (ROOT / "whisper" / "bind.py").read_text()
