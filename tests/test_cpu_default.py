"""CPU-only default. These fail if CUDA is auto-selected."""
...
def _toy_checkpoint(path):
    ...
    model = Whisper(dims)
    torch.save({"dims": asdict(dims), "model_state_dict": model.state_dict()}, path)
    return path
...
def test_load_model_local_checkpoint_lands_on_cpu(tmp_path, monkeypatch):
    monkeypatch.delenv(DEVICE_ENV, raising=False)
    monkeypatch.setattr(torch.cuda, "is_available", lambda: True)
    model = whisper.load_model(str(_toy_checkpoint(tmp_path / "toy.pt")))
